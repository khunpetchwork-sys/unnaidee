...
export default function ContentsGrid({ contents, productsByContent }) {
  ...
  if (contents.length === 0) {
    return <p className="text-center text-inkSoft text-sm mt-10">ยังไม่มีคอนเทนต์ในตอนนี้</p>;
  }
  ...
  return (
    <div>
      <div className="grid grid-cols-2 gap-3">
        {contents.map((c) => {
          const isArticle = c.content_type === 'article';
          const prodCount = (productsByContent[c.id] || []).length;
          ...
          return (
            ...
            >
              ...
              <div className="relative w-full" style={{ paddingBottom: '60%' }}>
                <div className="absolute inset-0 overflow-hidden" style={{ backgroundColor: isArticle ? '#F7F6F2' : '#1C1B18' }}>
                  {c.thumbnail_url ? (
                  ...
                  {/* badge บนรูป */}
                  {isArticle ? (
                    <span className="absolute top-1.5 left-1.5 bg-white/90 text-ink text-[9px] font-semibold px-1.5 py-0.5 rounded">
                      📝 บทความ
                    </span>
                  ) : (
                    <span
